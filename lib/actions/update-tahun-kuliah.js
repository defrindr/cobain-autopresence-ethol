...
module.exports = async(ClientRequest, token) => {
    ClientRequest.defaults.jar.setCookie("token=" + token + "; Path=/; Secure;", Config.url.BASE_URL);
    let headers = {
        token: `${token}`,
    ...
    try {
        ...
        WriteFile(Config.path.BASE_PATH + Config.path.ETHOL_CONF_PATH, "var a, b, c, d;module.exports = " + response);
